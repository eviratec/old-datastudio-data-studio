...
angular.module('MyApp', ['ngMaterial', 'ngMessages', 'material.svgAssetsCache']).config(function ($mdThemingProvider) {
...
}).service('entityService', function ($scope) {}).controller('AppCtrl', function ($scope, $timeout, $mdSidenav) {
  ...
  this.$entities.forEach(function (entity) {
    entity.Attributes.forEach(function (attribute) {
      var cellRef = '#/U/1/T/' + $this.$type.ClassName + '/E/' + entity.ID + '/A/' + attribute.AttributeKey;
  ...
  this.showCreateDialog = function ($event) {
    ...
    $mdDialog.show({
      parent: parentEl,
      targetEvent: $event,
      template: '<md-dialog layout-padding="">' + '<h2>Create New <span ng-bind="$type.Label" style="font-weight:700"></span></h2>' + '</md-dialog>',
      locals: {
        $type: $this.$type
      },
